...
const Custom = ({ socket }: CustomProps) => {
  ...
  useEffect(() => {
    socket.on("user_data",(rooms:[], roomNumber: string)=>{
      ...
      setCreator(rooms[room?room:roomNumber].creator);
  ...
  console.log(creator,'this is creator of this room-->username', username);

  useEffect(() => {
    socket.on("user_joined", (updatedUserList: string[]) => {
      console.log(roomsArray,'this is roomsArray')
      setCreator(roomsArray[room?room:id]?.creator)
